...
public class LadybugPluginStreamAlt2 : LadybugPluginInterface {
	...
	override public void Start () {
		...
		CreateTextureAndPassToPlugin((int)ScreenMode.StreamAlt);	
	...
	{	
		MovingFrame();	
		UpdateImage ();
		displayLadybugImages(true);
		
		PluginRender.SetStreamAltRenderStatus (true);
		foreground = true;
	}
	
	override public void SetBackGround()
	{
		PluginRender.SetStreamAltRenderStatus (false);
		foreground = false;
	}
	...
	private void MovingFrame()
	{
		...
		{
			actualElapsed =  AudioCtl.GetCurAudioTime() + curStreamData.audio_latency;
