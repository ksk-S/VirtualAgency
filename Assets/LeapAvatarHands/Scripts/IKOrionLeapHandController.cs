...
Enable the use of a game avatar characters own hands instead of disembodied virtual hands when using the Leap Motion controller.
...
namespace LeapAvatarHands
{
    ...
    public class IKOrionLeapHandController : MonoBehaviour
    {
        public LeapRecorder recorder_ = new LeapRecorder();


        [Tooltip("The animator that drives the avatar and performs the IK pass.")]
        public Animator animator;
        
        // private GameObject rootNode;                            //the root, parent, gameobject that this script ultimately lives on.

        [Tooltip("Is the IK currently active?")]
        public bool ikActive = true;
        [Tooltip("Is the left hand currently active?")]
        public bool leftActive = false;
        [Tooltip("Is the right hand currently active?")]
        public bool rightActive = false;

        [Tooltip("The left hand of the avatar. You must set this!")]
        public GameObject avatarLeftHand;   //keeps track of the left hand gameobject for IK
        [Tooltip("The right hand of the avatar. You must set this!")]
        ...
        [Tooltip("How many seconds it takes to lerp back to default pose after a hand goes inactive.")]
        ...
        [Tooltip("The physics left hand")]
        public RigidIKHand leftPhysicsHand;
        [Tooltip("The physics right hand")]
        public RigidIKHand rightPhysicsHand;

        [Tooltip("The LeapProvider - should be a LeapXRProvider for VR headmounted mode. If unset, will search for it on the main camera.")]
        ...
        protected void Start()
        {
            ...
            if (provider == null)
                provider = Camera.main.GetComponent<LeapProvider>();
            if (provider == null)
            {
                Debug.LogError("IKOrionLeapHandController::Start::No Leap Provider component was set or found. It should typically be on the Camera.");
        ...
        private void SetHandTargets()
        {
            if (leftActive && leftHand != null)
            {
                lTarget = leftHand.transform.position;
            ...
            if (rightActive && rightHand != null)
            {
                rTarget = rightHand.transform.position;
        ...
        void UpdateHandRepresentations()
        {
            ...
            Frame new_frame = recorder_.updateFrame(provider.CurrentFrame);

            foreach (Leap.Hand curHand in new_frame.Hands)
            {
                if (curHand.IsLeft)
                {
