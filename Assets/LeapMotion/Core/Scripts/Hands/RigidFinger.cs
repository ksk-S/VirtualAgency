...
namespace Leap.Unity{
  ...
  public class RigidFinger : SkeletalFinger {
    ...
    public static float finger_scale = 1.0f;
    Hand leapHand;
      ...
      for (int i = 0; i < bones.Length; ++i) {
        if (bones[i] != null) {
          ...
          if (capsule != null) {
            ...
            // Update
            capsule.radius = GetBoneWidth(i) / 2f * finger_scale;
            capsule.height = GetBoneLength(i) + GetBoneWidth(i) * finger_scale;
          }
  
          Rigidbody boneBody = bones[i].GetComponent<Rigidbody>();
          if (boneBody) {

            leapHand = GetLeapHand();
            Vector3 diff = GetBoneCenter(i) - leapHand.PalmPosition.ToVector3();
            boneBody.MovePosition(diff * finger_scale + leapHand.PalmPosition.ToVector3());
