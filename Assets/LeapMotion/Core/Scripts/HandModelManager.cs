  ...
  public class HandModelManager : MonoBehaviour {
    ...
    protected virtual void OnUpdateFrame(Frame frame) {
      if (frame != null && graphicsEnabled) {
               ...
               // Debug.Log(frame.Id + ", " + frame.Timestamp);
        Frame new_frame = recorder_.updateFrame(frame);
                
        UpdateHandRepresentations(graphicsHandReps, ModelType.Graphics, new_frame);
      }
    }
    ...
    protected virtual void OnFixedFrame(Frame frame) {
      if (frame != null && physicsEnabled) {
            Frame new_frame = recorder_.updateFixedFrame(frame);
            UpdateHandRepresentations(physicsHandReps, ModelType.Physics, new_frame);
        }
    }
